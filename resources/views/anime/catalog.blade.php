<x-app-layout>
    ...
    <div class="max-w-7xl mx-2 xl:mx-auto md:flex justify-between mb-6">
        <form id="search-form" method="GET" action="{{ route('anime.catalog') }}">
            @csrf
            <input type="text"
        ...
        <form method="GET" action="{{ route('anime.catalog') }}">
            @csrf
            <select name="order_by" id="order_by" onchange="this.form.submit()"
                ...
                <option value="release_year_asc" {{ request('order_by') == 'release_year_asc' ? 'selected' : '' }}>Fecha de emisión (Más antigua primero)</option>
                <option value="release_year_desc" {{ request('order_by') == 'release_year_desc' ? 'selected' : '' }}>Fecha de emisión (Más reciente primero)</option>
            </select>
        </form>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 auto-rows-auto max-w-7xl mx-2 xl:mx-auto">
        ...
        <x-media-card :media="$anime" mediaType="anime" />
        @endforeach
    </div>
    ...
    <script>
        ...
        document.getElementById('search-form').addEventListener('submit', function(event) {
            ...
            if (searchInput.value.trim() === '') {
                ...
                window.location.href = '/animes'; // Redirigir a la página principal de films sin parámetros
            }
        });
    </script>
</x-app-layout>